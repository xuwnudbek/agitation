...
class AboutOrderPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<AboutOrderProvider>(
        create: (context) => AboutOrderProvider(id: id, workerId: user["id"]),
        builder: (context, snapshot) {
          var user = jsonDecode(Hive.box("db").get("user"));
          return Scaffold(
            bottomNavigationBar: user["job_title"] != 1 //if job title equals 1 the worker is Admin
                ? null
                : Padding(
